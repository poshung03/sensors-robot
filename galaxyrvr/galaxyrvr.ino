...
void setup() {
  ...
  SoftPWMSet(2, 200);  // left motor forward
  SoftPWMSet(3, 0);

  SoftPWMSet(4, 200);  // right motor forward
  SoftPWMSet(5, 0);
...
void modeHandler() {
  switch (currentMode) {
    ...
    case MODE_VOICE_CONTROL:
      ...
      voice_control();
      break;
    //case MODE_DETECTHOSTILES:
    //  rgbWrite(MODE_DETECTHOSTILES_COLOR)
    //  break;
    default:
      break;
  }
...
void onReceive() {
  ...
  if (aiCam.getSwitch(REGION_E)) {
    ...
    if (currentMode == MODE_OBSTACLE_FOLLOWING || currentMode == MODE_OBSTACLE_AVOIDANCE) {
      currentMode = MODE_NONE;
      carStop();
      Serial.println("Switched to Mode None");
      return;
    }
  }
  ...
  // throttle
  int throttle_L = aiCam.getThrottle(10);
  int throttle_R = aiCam.getThrottle(16);
  ...
  // Detect Hostiles Code
  //void detecthostiles
  //break
  Serial.print("A ");
  Serial.print(aiCam.getThrottle(REGION_A));
  Serial.print(" B ");
  Serial.print(aiCam.getThrottle(REGION_B));
  Serial.print(" C ");
  Serial.print(aiCam.getThrottle(REGION_C));
  Serial.print(" D ");
  Serial.print(aiCam.getThrottle(REGION_D));
  Serial.print(" E ");
  Serial.print(aiCam.getThrottle(REGION_E));
  Serial.print(" F ");
  Serial.print(aiCam.getThrottle(REGION_F));
  Serial.print(" G ");
  Serial.print(aiCam.getThrottle(REGION_G));
  Serial.print(" H ");
  Serial.println(aiCam.getThrottle(REGION_H));
  Serial.print(" I ");
  Serial.print(aiCam.getThrottle(REGION_I));
  Serial.print(" J ");
  Serial.print(aiCam.getThrottle(REGION_J));
  Serial.print(" K ");
  Serial.print(aiCam.getThrottle(REGION_K));
  Serial.print(" L ");
  Serial.println(aiCam.getThrottle(REGION_L));
  Serial.print(" M ");
  Serial.print(aiCam.getThrottle(REGION_M));
  Serial.print(" N ");
  Serial.print(aiCam.getThrottle(REGION_N));
  Serial.print(" O ");
  Serial.print(aiCam.getThrottle(REGION_O));
  Serial.print(" P ");
  Serial.println(aiCam.getThrottle(REGION_P));
  Serial.print(" Q ");
  Serial.println(aiCam.getThrottle(REGION_Q));
}
